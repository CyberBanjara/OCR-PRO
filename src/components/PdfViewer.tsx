...
import { renderPage, renderTextLayer } from '@/lib/pdf-renderer';
  ...
  useEffect(() => {
    ...
    renderPage(pageNumber, canvas, scale)
      .then(async () => {
        ...
        // Render the text layer on top of the canvas for native selection
        if (textContainer) {
          try {
            await renderTextLayer(pageNumber, textContainer, scale);
          } catch (e) {
            console.warn('[PdfViewer] Text layer rendering failed:', e);
          }
        }
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        >
          ...
          {showTextLayer ? 'Text Selection ON' : 'Text Selection OFF'}
        </button>
      ...
      {/* Page with text layer + column overlay */}
      <div ref={containerRef} className="relative inline-block">
        ...
        <div
          ref={textLayerRef}
          className="pdfjs-text-layer absolute inset-0 w-full h-full rounded-lg"
          style={{
            display: loading ? 'none' : 'block',
            // Hide text layer visually but keep it selectable when disabled
